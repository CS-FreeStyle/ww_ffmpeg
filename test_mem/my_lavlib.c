...
int write_image_to_file(FILE *file, const char *filename, int count, AVFrame *frame, AVCodecContext *codec_context, AVPacket *pkt) {
  ...
  for (i = 0; i < count; i++) {
    if((res = write_frame_to_file(file, frame, codec_context, pkt)) < 0){
...
//   avcodec_register_all();
  ...
  if((codec = avcodec_find_encoder(AV_CODEC_ID_H265)) == NULL){
      fprintf(stderr,"unable to find codec\n");
      goto error;
  }
  ...
  // avcodec_open2(codec_context, videoCodec, &codec_options);
  ...
  if((res = avcodec_open2(codec_context, codec, &codec_options)) < 0){
